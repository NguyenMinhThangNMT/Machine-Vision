...
#Load Model 
model = None  # Khai báo biến model là biến toàn cục
...
def create_new_window1():
    ...
    class Emotion_Detect:        
        def __init__(self, window):
            ...
            # Khởi tạo camera
            self.cap = cv2.VideoCapture('http://192.168.1.38:4747/video')
    ...
    camera = Emotion_Detect(new_window)
...
def RECORD():
    Wrecord = Toplevel(login)
    Wrecord.title("XÁC ĐỊNH CẢM XÚC DỰA TRÊN HÌNH ẢNH")
    Wrecord.geometry("1000x600")
    mainWindow.withdraw()

    logo = Image.open('logonews.png')
    resized_image = logo.resize((354, 81))
    a = ImageTk.PhotoImage(resized_image)
    image_label = Label(Wrecord, image=photo)
    image_label.place(x=10,y=10)

    ckm = Image.open('CKM.png')
    resized_image = ckm.resize((100, 100))
    b = ImageTk.PhotoImage(resized_image)
    image_label1 = Label(Wrecord, image=photo1)
    image_label1.place(x=845,y=10)
    ...
    def close_new_window():
        ...
        mainWindow.deiconify()
    # Tạo một nút trong cửa sổ mới để đóng cửa sổ hiện tại
    close_button = Button(Wrecord, text="Back",height=1,width=12, command=close_new_window)
    close_button.place(x = 40, y = 100)
...
B1 = Button(login,text="EMOTION RECOGNITION",height=2,width=20,command= create_new_window1 )
B1.place(x =400,y=200)
B5 = Button(login,text="RECORD",height=2,width=20,command= R)
