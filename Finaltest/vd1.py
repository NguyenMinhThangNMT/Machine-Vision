...
def detectRec():
    ...
    op = cv2.VideoWriter('videorec.mp4', fourcc, 15.0, (640, 480))
    while cap.isOpened():
        ret, frame = cap.read()
        height, width, channel = frame.shape
        gray_image = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        ...
        for (x, y, w, h) in faces:
                cv2.rectangle(frame, (x, y), (x+w, y+h), (0, 255, 0), 2)
                # cắt khung hình chứa khuôn mặt
                cropped_img = frame[y:y + h, x:x + w]
                #  resize lại kích thướt ảnh
                resized_img = cv2.resize(cropped_img, (40, 40))
                # chuyển ảnh thành mảng numpy
                imgnumpy = img_to_array(resized_img)       
                arrayImage = imgnumpy.reshape(1, 40,40, 3) / 255.0                
                pred = np.argmax(model.predict(arrayImage))
                class_name = {0: 'Surprise', 1: 'Sad', 2: 'Neutral', 3: 'Happy', 4: 'Fear', 5: 'Angry', 6: 'Disgust'}        
                cv2.putText(frame, class_name[pred], (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (36, 255, 12), 2)
...
image_label1_1.place(x=840,y=10)
...
B1.place(x =0,y=200)
B5 = Button(login,text="RECORD",height=2,width=20,command= detectRec)
B5.place(x =0,y=250)
...
B4.place(x =0,y=400)
B4 = Button(login,text="LOAD MODEL",height=2,width=20,command= loadmodel )
B4.place(x =0,y=450)
